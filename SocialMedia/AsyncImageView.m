...
-(void) setStopActivityView :(BOOL)style
{
    ...
    if (self.showActivityIndicator && !self.image && imageURL)
    {
        if (self.activityView == nil)
        {
            ...
            if(self.bounds.size.width != 0){
                ...
                self.activityView.center = CGPointMake([UIScreen mainScreen].bounds.size.width/ 2.0f, self.bounds.size.height / 2.0f);
...
{
    if(self.FeedType == 1 && !self.bNormalShow){
        ...
        if(self.frame.size.width == 0){
            ...
            rt.size.width = image.size.width*self.tempscroll.frame.size.height/image.size.height;
            self.frame = rt;
            if(self.FeedType == 1 && self.bNormalShow)
                self.tempscroll.contentSize = CGSizeMake(rt.size.width, 0);
        }
    }
    super.image = image;
}
